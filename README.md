...
This is a (still unfinished) rewrite of Seismon's QuakeC interpreter to be a reusable module that
can be included like any `bevy_mod_scripting` language. The main part of the design that deviates
is the use of managed values instead of every value being an index. It's not possible to entirely
avoid indices, since that's ultimately what the format uses internally, but QuakeC is surprisingly
strongly-typed and so we can make the representation pretty opaque.
